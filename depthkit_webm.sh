...
ffmpeg -ss $START_SECOND -i "${INPUT_MOVIE_PATH}" -t $DURATION_IN_SECONDS -ac 2 -ar 44100 -vn "${INPUT_MOVIE_PATH}.WAV" 

echo "EXTRACTING POSTER FRAME...."
ffmpeg -ss $START_SECOND -i "${INPUT_MOVIE_PATH}" -frames:v 1 "{$OUTPUT_MOVIE_PATH}.png"

echo "EXTRACTING BUILDING WEBM...."
ffmpeg -start_number $START_FRAME -f image2 -r ${FRAME_RATE} -i "${SEQUENCE_PATH}/save.%05d.png" -i "${INPUT_MOVIE_PATH}.WAV" -codec:v libvpx -crf 10 -b:v 1M -c:a libvorbis -r $FRAME_RATE "${OUTPUT_MOVIE_PATH}.webm"
